...
Definicion (Experimento) : Un experimento es un proceso que genera observaciones y puede ser repetido. (Y en caso de no poder ser repetido?)
...
Definicion (Partición del espacio muestral) : Una partición de eventos $A_1, \ldots, A_k$ es una partición del espacio muestral S si
...
Definicion (Eventos elementales) : Sea S un espacio muestral finito o infinito numerable. Se definen eventos elementales a los {E} con E $\in$ S.
...
Definicion (Espacio equiprobable) : Sea un experimento aleatorio con espacio muestral S. Sea n = #S.
                                    Decimos que S es equiprobable si P($E_i$) = p   $\forall i : \mathbb{N}$
...
Definicion (Eventos independientes) : Dados dos eventos A, B de un espacio. Decimos que los eventos son independientes si y solo si
                                      P(A $\cap$ B) = P(A).P(B)
